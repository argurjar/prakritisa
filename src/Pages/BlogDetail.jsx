          ...
          <div className="blog_detail_wrapper">
            ...
            <div className="blog_detail_img">
              ...
              <div className="blog_inner_content">
                ...
                <div
                  className='blog_para_content'
                  dangerouslySetInnerHTML={{ __html: currentBlog.blogDetail }}
                ></div>
            ...
            <div className="blog_posts">
              ...
              <ul>
                ...
                {relatedBlogs.map((item) => (
                  <li key={item._id} className='related_blogs'>
                    <Link to={`/blog-details/${item.slug}`} className='related_blog_box'>
                      <img
                        src={`${import.meta.env.VITE_BACKEND_URL}${item.blogImg.url}`}
                        alt={item.blogName}
                        width="60px"
                        style={{ marginRight: '10px' }}
                      />
                      <div>
                        <p className="" style={{ justifyContent: 'left' }}>
                          <CgCalendarDates /> &nbsp;{new Date(item.blogDate).toISOString().split("T")[0]}
                        </p>
                        <p className='related_blog_title'>{item.blogName}</p>
                      </div>
                    </Link>
